...
//構造体Car,Node,Linkははそれぞれのヘッダファイルで定義
//Car Cars, Node Nodes, Link Linksはmainで定義したグローバル変数
...
void initCar()
{
  for (std::vector<Car>::iterator itr = Cars.begin(); itr != Cars.end(); itr++)
  {
      itr ->isUsed = 0;
      itr ->canTransferLinks = 1;
      //これで回せている！！
  }
}
